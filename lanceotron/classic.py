...
def label_enriched_regions_threshold(array, threshold, min_region_size=0, max_region_size=None):
...
def label_enriched_regions_dynamic_threshold_width(array, threshold, increment, max_width, min_region_size=0, return_intermediate_coords=False, recurse=False, level=0):
    level+=1
    enriched_region_list = label_enriched_regions_threshold(array, threshold, min_region_size)
    out_list = []
    new_regions = []
    corrected_list = []
            ...
            if region_size>max_width:
                if level<=50:
                    ...
                    new_regions = label_enriched_regions_dynamic_threshold_width(
                        array[start_coord:end_coord], threshold+increment, increment, max_width, min_region_size=min_region_size, return_intermediate_coords=return_intermediate_coords, recurse=True, level=level)
                    if new_regions:
                        for region in new_regions:
                            corrected_list.append([region[0]+start_coord, region[1]+start_coord])
                    else:
                        out_list.append([start_coord, end_coord])
                else:
                    out_list.append([start_coord, end_coord])
...
def select_best_savgol_window(array, window_list):
    ...
    while not blank_signal_coord_list_filtered:
        enriched_regions_list = label_enriched_regions_threshold(smooth_array, fold_mean_enrichment*np.mean(array))
        ...
        if len(enriched_regions_array) > 2:
            ...
            for x in it:
                ...
                if (region_size>2000) and (region_size<10000):
                    blank_signal_coord_list.append([left_coord, right_coord, region_size])
        else:
    ...
    for lag in lag_list:
        ...
        for coords in blank_signal_coord_list_filtered:            
            coeff = np.corrcoef(array[coords[0]:coords[1]-lag], array[coords[0]+lag:coords[1]])[0][1]
    ...
    for window_size in window_list:
        autocorcoef_sum = 0
        for coords in blank_signal_coord_list_filtered:
            savgol_window_test_array_smooth = savgol_filter(array[coords[0]:coords[1]], window_size, 2)
            savgol_window_test_array_res = array[coords[0]:coords[1]]-savgol_window_test_array_smooth
            input_padding = int((window_size-1)/2)
            coeff = np.corrcoef(savgol_window_test_array_res[input_padding:-input_padding-1], savgol_window_test_array_res[input_padding+1:-input_padding])[0][1]
            autocorcoef_sum+=(coeff*coords[2])/blank_signal_length
...
def label_enriched_regions_savgol(array, window_len, min_region_size, polynomial=2, window_list=None, max_region_size=None, threshold=0):
    if window_len == 'auto':
        if window_list:
    ...
    for coords in enriched_region_list:
        ...
        if (region_size>=min_region_size) and (region_size<=max_region_size) and (max_height>threshold):
            filtered_region_list.append(coords)
    return filtered_region_list

def extract_signal_single(pyBigWig_object, chrom_name, start_coord, end_coord, chrom_len, features, zero_pad=True):
    X = np.zeros(features)
    region_length = end_coord-start_coord
    if ((start_coord+int(region_length/2)-int(features/2))>=0) and ((start_coord+int(region_length/2)-int(features/2)+features)<=chrom_len):
        if zero_pad:
            if region_length>features:
                region_start = start_coord+int(region_length/2)-int(features/2)
                X = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start, region_start+features)))
            else:
                pad_length = int((features-region_length)/2)
                X[pad_length:pad_length+region_length] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, start_coord, end_coord)))
        else:
            region_start = start_coord+int(region_length/2)-int(features/2)
            X = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start, region_start+features)))
    return X
...
def extract_signal_wide_and_deep_single_entry(pyBigWig_object, chrom_name, start_coord, end_coord, chrom_len, chrom_mean, seq_depth):
    features = 2000
    ...
    max_height = pyBigWig_object.stats(chrom_name, start_coord, end_coord, type='max', exact=True)[0]/seq_depth
    if (max_height>0):
        region_start = start_coord+int(region_length/2)-int(features/2)
        if (region_start>=0) and (region_start+features<=chrom_len):
            X_deep = (np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start, region_start+features))))/seq_depth
        ...
        if (start_coord+int(region_length/2)-lambda_cov>=0) and (start_coord+int(region_length/2)+lambda_cov<=chrom_len):
            lambda_cov_array = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, start_coord+int(region_length/2)-lambda_cov, start_coord+int(region_length/2)+lambda_cov)))/seq_depth
            ...
            for i, length in enumerate(length_list):
                if (start_coord+int(region_length/2)-length>=0) or (start_coord+int(region_length/2)+length<=chrom_len):
                    ...
                    lambda_entry = np.nan_to_num(np.array(pyBigWig_object.stats(chrom_name, start_coord_for_mean, end_coord_for_mean, type='mean', exact=True)))
...
def extract_signal_single_dl_input1(pyBigWig_object, chrom_name, start_coord, end_coord, features):
    X = np.zeros((features, 3))
    region_length = end_coord-start_coord
    if region_length>features:
        region_start = start_coord+int(region_length/2)-int(features/2)
        X[:,0] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start, region_start+features)))
        X[:,1] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start-features, region_start)))
        X[:,2] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, region_start+features, region_start+(2*features))))
    else:
        pad_length = int((features-region_length)/2)
        X[pad_length:pad_length+region_length,0] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, start_coord, end_coord)))
        X[:,1] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, start_coord-features, start_coord)))
        X[:,2] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, end_coord, end_coord+features)))
    return X

def extract_signal_single_dl_input2_old(pyBigWig_object, chrom_name, start_coord, end_coord, chrom_mean, chrom_std, kb_to_collect_stats=20):
    ...
    region_coverage = np.nan_to_num(np.array(pyBigWig_object.values(chrom_name, (mid-half_range), (mid-half_range)+full_range)))
    region_coverage = (region_coverage-chrom_mean)/chrom_std
    
    X[0,0] = np.mean(region_coverage[start_coord-(mid-half_range):end_coord-(mid-half_range)])
    X[0,1] = np.std(region_coverage[start_coord-(mid-half_range):end_coord-(mid-half_range)])
    
    for i, region_length in enumerate(range(1000, (1000*kb_to_collect_stats)+1000, 1000)):
        if i > 0:
            half_length = int(region_length/2)
            X[i,0] = np.mean(region_coverage[half_range-half_length:(half_range-half_length)+region_length])
            X[i,1] = np.std(region_coverage[half_range-half_length:(half_range-half_length)+region_length])
    return X

def extract_signal_single_dl_input2(pyBigWig_object, chrom_name, start_coord, end_coord, chrom_len, chrom_mean, chrom_std, seq_depth, kb_to_collect_stats=20):
...
class Bigwig_data():
    ...
    def get_chrom_info_make_coverage_map(self, chrom_name, smoothing=None, savgol=False, return_chrom_stats_dict=False):
        chrom_stats_dict = self.get_chrom_info(chrom_name)
        chrom_coverage_array = self.make_chrom_coverage_map(chrom_stats_dict, smoothing, savgol)
        if return_chrom_stats_dict:
            return chrom_coverage_array, chrom_stats_dict
        else:
            return chrom_coverage_array
        
    def make_chrom_coverage_map_label_enriched_regions_threshold(self, chrom_stats_dict, threshold, min_region_size=0, max_region_size=None, smoothing=None, savgol=False, return_chrom_coverage_array=False):
        chrom_coverage_array = self.make_chrom_coverage_map(chrom_stats_dict, smoothing, savgol)
        enriched_regions_list = label_enriched_regions_threshold(chrom_coverage_array, threshold, min_region_size, max_region_size)
        if return_chrom_coverage_array:
            return enriched_regions_list, chrom_coverage_array
        else:
            return enriched_regions_list
                
    def make_chrom_coverage_map_label_enriched_regions_savgol(self, chrom_stats_dict, smoothing='auto', min_region_size=0, max_region_size=None, threshold=0, return_chrom_coverage_array=False):
        chrom_coverage_array = self.make_chrom_coverage_map(chrom_stats_dict)
        enriched_regions_list = label_enriched_regions_savgol(chrom_coverage_array, smoothing, min_region_size, max_region_size, threshold=threshold)
        if return_chrom_coverage_array:
            return enriched_regions_list, chrom_coverage_array
        else:
            return enriched_regions_list
    
    def get_chrom_info_make_chrom_coverage_map_label_enriched_regions_threshold(self, chrom_name, threshold, min_region_size=0, max_region_size=None, smoothing=None, savgol=False, return_chrom_coverage_array_stats_dict=False):
        if return_chrom_coverage_array_stats_dict:
            chrom_coverage_array, chrom_stats_dict = self.get_chrom_info_make_coverage_map(chrom_name, smoothing, savgol, return_chrom_stats_dict=True)
        else:
            chrom_coverage_array = self.get_chrom_info_make_coverage_map(chrom_name, smoothing, savgol)
        enriched_regions_list = label_enriched_regions_threshold(chrom_coverage_array, threshold, min_region_size, max_region_size)
        if return_chrom_coverage_array_stats_dict:
    ...
    def get_chrom_info_make_chrom_coverage_map_label_enriched_regions_savgol(self, chrom_name, smoothing='auto', min_region_size=0, max_region_size=None, threshold=0, return_chrom_coverage_array=False):
        chrom_coverage_array = self.get_chrom_info_make_coverage_map(chrom_name)
        enriched_regions_list = label_enriched_regions_savgol(chrom_coverage_array, smoothing, min_region_size, max_region_size)
        if return_chrom_coverage_array:
            return enriched_regions_list, chrom_coverage_array
        else:
            return enriched_regions_list

    def extract_signal_chrom(self, chrom_stats_dict, coord_list, features, normalize=True, zero_pad=True):
        ...
        for i, coord in enumerate(coord_list):
            ...
            if zero_pad:
                if region_length>features:
                    ...
                    X[i] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_stats_dict['chrom_name'], region_start, region_start+features)))
                else:
                    pad_length = int((features-region_length)/2)
                    X[i,pad_length:pad_length+region_length] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_stats_dict['chrom_name'], coord[0], coord[1])))
            else:
                region_start = coord[0]+int(region_length/2)-int(features/2)
                X[i] = np.nan_to_num(np.array(pyBigWig_object.values(chrom_stats_dict['chrom_name'], region_start, region_start+features)))
        if normalize:
