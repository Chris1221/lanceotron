...
def cli():
    ...
    findandscore.add_argument('--format', type=str, default = "web", help = "Format of the output file. 'Web' will mimic the output from the web app, 'Bed' will create a standard three column bed file.")
    ...
    fas_input = subparsers.add_parser("callPeaksInput", help = "Call peaks from a bigWig file with an input track for control.")
    ...
    score = subparsers.add_parser("scoreBed", help = "Score an existing bed file using Lanceotron's model and a coverage track")
    score.add_argument('file', help='bigwig file')
    score.add_argument('-b', '--bed', type=str, help='bed file of regions to be scored using L-tron\'s neural network')
    score.add_argument('-f', '--folder', type=str, default='./', help='folder to write results to; default=current directory')
    score.add_argument('--skipheader', default=False, action='store_true', help='skip writing header')
